...
- 維度表 (Dimension Tables)：將商業意圖、產業分類與地理資訊標準化。
- 事實表 (Fact Tables & Views)：對接 Power BI 等前端 BI 工具的關聯模型。

### 2. 高併發與防鎖死機制 (Concurrency & Deadlock Prevention)
- 捨棄傳統容易引發 Table Lock 的 MERGE 語法，改用 UPDATE 與 INSERT ... WHERE NOT EXISTS 寫入邏輯，確保多次執行不會產生重複資料。
...
- 開發智慧補抓邏輯，確保自動化排程 (Airflow) 只針對尚未成功的目標進行重試，不破壞歷史錯誤現場。
